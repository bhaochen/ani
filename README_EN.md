...
User data is rooted at `ANI_HOME` (`~/.ani` in production, `~/.ani-dev` in development). Hana-managed Pi SDK runtime resources live under `${ANI_HOME}/runtime/pi-sdk/`; Hana does not rely on Pi's global agent directory or `PI_CODING_AGENT_DIR`. Legacy `fd` / `rg` binaries under `${ANI_HOME}/.pi/agent/bin/` are copied into the new directory on first use of the corresponding search tool, while the legacy files remain untouched.
