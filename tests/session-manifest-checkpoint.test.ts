...
describe("session manifest migration checkpoint", () => {
  ...
  beforeEach(() => {
    aniHome = fs.mkdtempSync(path.join(os.tmpdir(), "hana-manifest-checkpoint-"));
  });
  ...
  function writeHomeFile(relativePath, content) {
    const fullPath = path.join(aniHome, relativePath);
  ...
  it("restores checkpoint data and moves aside the current manifest database files", () => {
    ...
    fs.writeFileSync(path.join(aniHome, "session-manifest.db"), "broken-db");
    fs.writeFileSync(path.join(aniHome, "session-manifest.db-wal"), "broken-wal");
    fs.writeFileSync(path.join(aniHome, "session-manifest.db-shm"), "broken-shm");
    ...
    expect(fs.existsSync(path.join(aniHome, "session-manifest.db"))).toBe(false);
    expect(result.movedManifestFiles).toEqual([
      {
        from: path.join(aniHome, "session-manifest.db"),
        to: path.join(aniHome, "session-manifest.db.rollback-2026-06-18T02-02-00-000Z"),
      },
      {
        from: path.join(aniHome, "session-manifest.db-wal"),
        to: path.join(aniHome, "session-manifest.db-wal.rollback-2026-06-18T02-02-00-000Z"),
      },
      {
        from: path.join(aniHome, "session-manifest.db-shm"),
        to: path.join(aniHome, "session-manifest.db-shm.rollback-2026-06-18T02-02-00-000Z"),
      },
    ]);
    expect(fs.readFileSync(path.join(aniHome, "session-manifest.db-wal.rollback-2026-06-18T02-02-00-000Z"), "utf-8")).toBe("broken-wal");
    expect(fs.readFileSync(path.join(aniHome, "session-manifest.db-shm.rollback-2026-06-18T02-02-00-000Z"), "utf-8")).toBe("broken-shm");
    expect(fs.existsSync(path.join(aniHome, "session-manifest.db-wal"))).toBe(false);
    expect(fs.existsSync(path.join(aniHome, "session-manifest.db-shm"))).toBe(false);
